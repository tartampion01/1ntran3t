...
%>
    ...
    <td width="33%" align="right" valign="middle">
		<a <%=print_action%> class="bt prn noprint">imprimer</a>
		<%if session("level")=1 or session("level")=2 then %><a href="details.asp" class="bt add noprint">ajouter</a><%End If%>
	</td>
  </tr>
</table>
...
%>
  <tr OnClick="document.location.href='details.asp?ID=<%=rs("emploi_id")%>&r=<%=Server.URLEncode(currentpage&"?page="&page)%>'" class='<%=TRStyle%>' OnMouseOver="this.className='over';" OnMouseOut="this.className='<%=TRStyle%>';">
    <%
For i = 0 to Ubound(Headers,1)
%>
