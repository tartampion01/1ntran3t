...
If RequestForm<>"" then
	...
	for each items in RequestForm("_img_file")
		...
		If theField.FileExists and theField.ImageType <> 0 Then
			...
			Conn.Execute("UPDATE inv_pictures SET intorder='" & pic_order & "', name='"&picfilename&"' WHERE ID ='"&picID&"';")
    ...
    <tr>
      ...
      <td class="cell_content">
		...
		<select name="model" id="model">
		  ...
		  <option <%if model="NRR Diesel" then response.write "selected"%>>NRR Diesel</option>
...
<div id="inv" style="display:none;" class="upload">
  ...
  <%end if%>
	...
	<tr>
      ...
      <td class="cell_content"><input name="serial" type="text" id="serial" value="<%=serial%>" maxlength="8"/></td>
    ...
    <tr>
      ...
      <td class="cell_content" id="allpics" colspan="3"><%
                  ...
                  <table border="0" cellspacing="0" cellpadding="0" style="float:left;margin:0 5px 5px 0;border:1px solid #666; background-color:#EEE;padding:1px;">
                    <tr>
                      <td colspan="2" align="center"><a data-lightbox="<%=rs("name")%>"  href="/intranet/pics/<%=rs("name")%>" target="_blank"><img src="/intranet/pics/small/<%=rs("name")%>" border="1" height="60" width="60" alt="Click for a larger image" /></a></td>
                    </tr>
					<%IF Session("Level")=1 then%>
                    <tr>
